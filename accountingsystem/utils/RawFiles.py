    ...
    try:
        with transaction.atomic():
            for i in range(1, sheet.nrows):
                ...
                ##建立model跟db
                record = AccountsPayable.objects.create(voucher_number = sheet.cell_value(rowx=i, colx=0) if not isinstance(sheet.cell_value(rowx=i, colx=1), (int, float)) else int(sheet.cell_value(rowx=i, colx=0)),
                                           voucher_date =  datetime(*xlrd.xldate_as_tuple(sheet.cell_value(rowx=i, colx=1), 0)),
                                           customer_code = sheet.cell_value(rowx=i, colx=2) if not isinstance(sheet.cell_value(rowx=i, colx=2), (int, float)) else int(sheet.cell_value(rowx=i, colx=2)),
                                           ...
                                           currency = currency,
                                            # 帳號欄位若讀成了「數字」型態，則去掉 .0，否則直接存入 DB
                                           foreign_currency_amount = Decimal(sheet.cell_value(rowx=i, colx=6)).quantize(Decimal('.01'), rounding=ROUND_05UP) if sheet.cell_value(rowx=i, colx=6) != '' else None,
