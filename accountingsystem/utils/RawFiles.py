...
from ..models import Cashinbanks, Depositaccount, Report, Account, Systemcode, ReceiptsInAdvance, AccountsPayable, \
    Accountreceivable, Allowanceforloss
...
def check_and_save_cash_in_banks(rpt_id, sheet):  # 參數：sheet 為 Excel 中的分頁
    ...
    col_types = [xlrd.XL_CELL_TEXT, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_NUMBER,
                 xlrd.XL_CELL_NUMBER]  # 上傳的檔案欄位長度應該 為 6
    #
        ...
        with transaction.atomic():
            for i in range(1, sheet.nrows):
                ...
                record = Cashinbanks.objects.create(bank_name=sheet.cell_value(rowx=i, colx=0),
                                                    ...
                                                    bank_account_number=sheet.cell_value(rowx=i,
                                                                                         colx=1) if not isinstance(
                                                        sheet.cell_value(rowx=i, colx=1), (int, float)) else int(
                                                        sheet.cell_value(rowx=i, colx=1)),
                                                    type=type,
                                                    currency=currency,
                                                    foreign_currency_amount=Decimal(
                                                        sheet.cell_value(rowx=i, colx=4)).quantize(Decimal('.01'),
                                                                                                   rounding=ROUND_05UP) if sheet.cell_value(
                                                        rowx=i, colx=4) != '' else None,
                                                    ntd_amount=Decimal(sheet.cell_value(rowx=i, colx=5)).quantize(
                                                        Decimal('.01'), rounding=ROUND_05UP),
                                                    rpt=rpt)
                record.save()
...
def check_and_save_deposit_account(rpt_id, sheet):  # 參數：sheet 為 Excel 中的分頁
    ...
    col_types = [xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_NUMBER,
                 xlrd.XL_CELL_NUMBER
        ...
        with transaction.atomic():
            for i in range(1, sheet.nrows):
                ...
                record = Depositaccount.objects.create(
                    ...
                    # 帳號欄位若讀成了「數字」型態，則去掉 .0，否則直接存入 DB
                    bank_account_number=sheet.cell_value(rowx=i, colx=1) if not isinstance(
                        sheet.cell_value(rowx=i, colx=1), (int, float)) else int(sheet.cell_value(rowx=i, colx=1)),
                    type=type,
                    currency=currency,
                    foreign_currency_amount=Decimal(sheet.cell_value(rowx=i, colx=4)).quantize(Decimal('.01'),
                                                                                               rounding=ROUND_05UP) if sheet.cell_value(
                        rowx=i, colx=4) != '' else None,
...
def check_and_save_receipts_in_advance(rpt_id, sheet):
    ...
    col_types = [xlrd.XL_CELL_TEXT, xlrd.XL_CELL_DATE, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT,
                 xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_TEXT]
    ...
    try:
        with transaction.atomic():
            ...
            for i in range(1, sheet.nrows):
                currency = Systemcode.objects.filter(code_type='幣別', code_name=sheet.cell_value(rowx=i,
                                                                                                colx=4)).first().system_code  # currency欄位存所屬幣別的system_code
                if currency is None:
                    raise ObjectDoesNotExist
                receipts_in_advance_list.append(ReceiptsInAdvance(
                    voucher_num=sheet.cell_value(rowx=i, colx=0) if not isinstance(sheet.cell_value(rowx=i, colx=0),
                                                                                   (int, float)) else int(
                        sheet.cell_value(rowx=i, colx=0)),
                    voucher_date=datetime(*xlrd.xldate_as_tuple(sheet.cell_value(rowx=i, colx=1), 0)),
                    customer_code=sheet.cell_value(rowx=i, colx=2) if not isinstance(sheet.cell_value(rowx=i, colx=2),
                                                                                     (int, float)) else int(
                        sheet.cell_value(rowx=i, colx=2)),
                    customer_abbre=sheet.cell_value(rowx=i, colx=3),
                    currency=currency,
                    foreign_currency_amount=Decimal(sheet.cell_value(rowx=i, colx=5)).quantize(Decimal('.01'),
                                                                                               rounding=ROUND_05UP) if sheet.cell_value(
                        rowx=i, colx=5) != '' else None,
...
def check_and_save_accounts_payable(rpt_id, sheet):
    ''' 檢查及儲存「應付帳款」 '''
    # 確認有此專案/報表 ID
    rpt = Report.objects.filter(rpt_id=rpt_id).first()
    if rpt is None:
        return {"status_code": 404, "msg": "無此專案/報表。"}

    # 確認 column 的名稱和個數是否一致
    expected_ncols = 9
    col_names = ['傳票編號', '傳票日期', '客戶代號', '客戶簡稱', '類型', '幣別', '外幣金額', '原幣金額', '摘要']
    col_types = [xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_DATE, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT,
                 xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_TEXT]
    if sheet.ncols != expected_ncols:
        return {"status_code": 422, "msg": "檔案欄位個數不符合格式。"}
    if col_names != sheet.row_values(rowx=0, start_colx=0, end_colx=sheet.nrows):  # TODO 之後要更彈性
        return {"status_code": 422, "msg": "檔案欄位名稱不符合格式。"}
    ...
    for i in range(expected_ncols):
        ...
        # 第 i 個 column 的 cell type 應該都是一樣的，並且應該要與 col_types[i] 相同
            ...
            print(i, 'cell_type_list', cell_type_list[0], 'col_types', col_types[i])
            return {"status_code": 422, "msg": "檔案欄位型態不符合格式。"}
    # 儲存資料：
    try:
        with transaction.atomic():
            for i in range(1, sheet.nrows):
                ...
                ##建立model跟db
                record = AccountsPayable.objects.create(
                    voucher_number=sheet.cell_value(rowx=i, colx=0) if not isinstance(sheet.cell_value(rowx=i, colx=1),
                                                                                      (int, float)) else int(
                        sheet.cell_value(rowx=i, colx=0)),
                    voucher_date=datetime(*xlrd.xldate_as_tuple(sheet.cell_value(rowx=i, colx=1), 0)),
                    customer_code=sheet.cell_value(rowx=i, colx=2) if not isinstance(sheet.cell_value(rowx=i, colx=2),
                                                                                     (int, float)) else int(
                        sheet.cell_value(rowx=i, colx=2)),
                    customer_name=sheet.cell_value(rowx=i, colx=3),
                    type=type,
                    currency=currency,
                    # 帳號欄位若讀成了「數字」型態，則去掉 .0，否則直接存入 DB
                    foreign_currency_amount=Decimal(sheet.cell_value(rowx=i, colx=6)).quantize(Decimal('.01'),
                                                                                               rounding=ROUND_05UP) if sheet.cell_value(
                        rowx=i, colx=6) != '' else None,
                    ntd_amount=sheet.cell_value(rowx=i, colx=7),
...
def check_and_save_accountreceivable(rpt_id, sheet):
    ...
    col_types = [xlrd.XL_CELL_TEXT, xlrd.XL_CELL_DATE, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_TEXT,
                 xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_TEXT, xlrd.XL_CELL_DATE]
    expected_ncols = len(col_names)
    #
    if sheet.ncols != expected_ncols:
        return {"status_code": 422, "msg": "檔案欄位個數不符合格式。"}
...
def check_and_save_allowanceforloss(rpt_id, sheet):
    ...
    row_types = [xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER,
                 xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER, xlrd.XL_CELL_NUMBER]
        ...
        with transaction.atomic():
            ...
            for i in range(1, sheet.ncols):
                ...
                allowence_for_loss_list.append(Allowanceforloss(
                    opening_balance=Decimal(sheet.cell_value(rowx=1, colx=i)).quantize(Decimal('.01'),
                                                                                       rounding=ROUND_05UP),
                    recognized_amt=Decimal(sheet.cell_value(rowx=2, colx=i)).quantize(Decimal('.01'),
                                                                                      rounding=ROUND_05UP),
                    reversal_amt=Decimal(sheet.cell_value(rowx=3, colx=i)).quantize(Decimal('.01'),
                                                                                    rounding=ROUND_05UP),
                    charge_off_amt=Decimal(sheet.cell_value(rowx=4, colx=i)).quantize(Decimal('.01'),
                                                                                      rounding=ROUND_05UP),
                    exchange_gain_loss=Decimal(sheet.cell_value(rowx=5, colx=i)).quantize(Decimal('.01'),
                                                                                          rounding=ROUND_05UP),
                    alw_for_loss_individual=Decimal(sheet.cell_value(rowx=6, colx=i)).quantize(Decimal('.01'),
                                                                                               rounding=ROUND_05UP),
                    alw_for_loss_consolidate=Decimal(sheet.cell_value(rowx=7, colx=i)).quantize(Decimal('.01'),
                                                                                                rounding=ROUND_05UP),
                    rpt=rpt))
            # 一次新增至資料庫中
